...
export class DatosBasicosService implements Resolve<any>{
   ...
   * @returns {Promise<any>}
   */
  private _getAspirante(id: number): Promise<any> {
    const params =
  ...
  private _editar(id: number, dato: any): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.put(`${environmentAlcanos.administracionPersonal}/api/HojaDeVidas/${id}`, dato)
  ...
  private _crear(dato: any): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.post(`${environmentAlcanos.administracionPersonal}/api/HojaDeVidas`, dato)
        .subscribe((response: any) => {
          resolve(response);
        }, reject);
    });
