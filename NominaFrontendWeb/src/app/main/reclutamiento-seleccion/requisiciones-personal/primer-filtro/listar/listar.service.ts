...
export class PrimerFiltroListarService {
  ...
  public buildFilter(filters): Promise<any> {
    ...
    if (this.urlFilters.hasOwnProperty('$filter')) {
      ...
      urlFilters.push(`${decodeURIComponent(`requisicionPersonalId`)} eq ${decodeURIComponent(this.id)} and estado ne 'Postulado' and estado ne 'Descartado'`);
  ...
  private _getCandidatos(params: any): Promise<any> {
    ...
    if (!params.hasOwnProperty('$filter')) {
      // tslint:disable-next-line: max-line-length
      filter = `$filter=requisicionPersonalId eq ${this.id} and estado ne 'Postulado' and estado ne 'Descartado' and estadoRegistro eq 'Activo'`;
    }
