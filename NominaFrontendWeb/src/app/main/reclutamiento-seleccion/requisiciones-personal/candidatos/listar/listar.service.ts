...
export class CandidatosListarService {
  ...
  public buildFilter(filters): Promise<any> {
    ...
    if (this.urlFilters.hasOwnProperty('$filter')) {
      ...
      urlFilters.push(`${decodeURIComponent(`requisicionPersonalId`)} eq ${decodeURIComponent(this.id)}`);
  ...
  private _getCandidatos(params: any): Promise<any> {
    ...
    if (!params.hasOwnProperty('$filter')) {
      filter = `$filter=requisicionPersonalId eq ${this.id} and estadoRegistro eq 'Activo'`;
    }
