...
export class ListarService implements Resolve<any> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
        ...
        if (decodeURIComponent(value) !== 'null') {
          switch (decodeURIComponent(key)) {
            case 'numeroDocumento':
              ...
              urlFilters.push(`contains(${decodeURIComponent(`numeroDocumento`)}, '${decodeURIComponent(value)}')`);
              break;
            case 'primerNombre':
              dataFilters[decodeURIComponent(key)] = decodeURIComponent(value);
    ...
    //   this.urlFilters.$orderBy = 'fechaInicio asc';
    // }
  ...
  private _getSolicitudVacaciones(params: any): Promise<any> {
    ...
    const url = `${environmentAlcanos.configuracionGeneral}/odata/LibroVacacionesConsolidados?$count=true&${toUrlEncoded(params)}`;
