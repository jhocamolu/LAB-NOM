...
import { TerminarService } from './terminar.service';
import * as moment from 'moment';
import { estadoVacacionesAlcanos } from '@alcanos/constantes/estado-vacaciones';

@Component({
  selector: 'solicitud-vacaciones-terminar',
...
export class TerminarComponent implements OnInit {
  ...
  constructor(
    ...
    this.form = this._formBuilder.group({
      ...
      fechaFinDisfrute: [null, [Validators.required]],
  ...
  guardarHandle(event, element): void {
    ...
    formValue.fechaFinDisfrute = moment(formValue.fechaFinDisfrute).format('YYYY-MM-DDTHH:mm:ssZ');
    this._service.estado(this.element.id, formValue)
      .then((resp) => {
        ...
        if (resp.status === 400 && 'errors' in error) {
          ...
          if ('fechaFinDisfrute' in resp.error.errors) {
            const errors = {};
            resp.error.errors.fechaFinDisfrute.forEach(element => {
              errors[element] = true;
            });
