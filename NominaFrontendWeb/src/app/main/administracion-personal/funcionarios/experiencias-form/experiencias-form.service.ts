...
export class ExperienciasService implements Resolve<any>{
  ...
  private getExperiencia(id: number): Promise<any> {
    const url = `${environmentAlcanos.administracionPersonal}/odata/experienciaLaborales/${id}`;
    return new Promise((resolve, reject) => {
      this._httpClient.get(url).subscribe((response: any) => {
  ...
  private _editar(id: number, dato: any): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.put(`${environmentAlcanos.administracionPersonal}/api/experienciaLaborales/${id}`, dato)
  ...
  private _crear(dato: any): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.post(`${environmentAlcanos.administracionPersonal}/api/experienciaLaborales`, dato)
        .subscribe((response: any) => {
          resolve(response);
        }, reject);
    });
