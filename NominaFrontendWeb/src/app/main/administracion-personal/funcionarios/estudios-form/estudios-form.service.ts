...
export class EstudiosService implements Resolve<any>{
  ...
  private getEstudio(id: number): Promise<any> {
    const params =
      `$expand=profesion`;
    const url = `${environmentAlcanos.administracionPersonal}/odata/funcionarioEstudios/${id}?${params}`;
    return new Promise((resolve, reject) => {
      this._httpClient.get(url).subscribe((response: any) => {
  ...
  private _editar(id: number, dato: any): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.put(`${environmentAlcanos.administracionPersonal}/api/funcionarioEstudios/${id}`, dato)
  ...
  private _crear(dato: any): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.post(`${environmentAlcanos.administracionPersonal}/api/funcionarioEstudios`, dato)
        .subscribe((response: any) => {
          resolve(response);
        }, reject);
    });
