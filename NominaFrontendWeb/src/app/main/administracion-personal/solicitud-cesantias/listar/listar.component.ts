...
@Component({
  selector: 'solicitud-cesantias-listar',
...
export class ListarComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = ['funcionario/numeroDocumento', 'funcionario/primerNombre', 'fechaSolicitud', 'estado', 'acciones'];
  ...
  crearHandle(event): void {
    this._router.navigate(
      ['/administracion-personal/solicitud-cesantias/crear'],
    );
  }
  ...
  aprobarHandle(event, element): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this._service.getSolicitudCesantias();
      }
    });
  }
  ...
  cancelarHandle(event, element): void {
    ...
    dialogRef.afterClosed().subscribe(confirm => {
      if (confirm) {
        const dato = this.estadoSolicitud.cancelada;
