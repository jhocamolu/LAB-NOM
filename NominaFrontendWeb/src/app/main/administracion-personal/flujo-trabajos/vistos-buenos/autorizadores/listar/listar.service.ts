...
export class AutorizadoresListarService {
  ...
  public buildFilter(filters): Promise<any> {
    ...
    if (this.urlFilters.hasOwnProperty('$filter')) {
      ...
      urlFilters.push(`${decodeURIComponent(`tipo`)} eq 'Autorizacion'`);
  ...
  private _getAutorizadores(params: any): Promise<any> {
    ...
    if (!params.hasOwnProperty('$filter')) {
      filter = `$filter= tipo eq 'Autorizacion' and aplicacionExternaId eq ${this.id}`;
    }
