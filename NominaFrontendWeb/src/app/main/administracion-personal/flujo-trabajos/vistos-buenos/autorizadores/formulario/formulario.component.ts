...
import { AutorizadoresFormularioService } from './formulario.service';
import { FormularioService } from '../../formulario/formulario.service';

@Component({
  selector: 'aprobaciones-autorizaciones-formulario',
  templateUrl: './formulario.component.html',
  styleUrls: ['./formulario.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class AutorizadoresFormularioComponent implements OnInit {
  ...
  coAutorizadorOptions: any;
  claseAprobador = claseAprobador;
  cargoAutorizadorOptions: any[];
  ...
  constructor(
    public dialogRef: MatDialogRef<AutorizadoresFormularioComponent>,
    @Inject(MAT_DIALOG_DATA) public element: any,
    private _formBuilder: FormBuilder,
    private _alcanosSnackBar: AlcanosSnackBarService,
    private _service: AutorizadoresFormularioService,
    ...
    this.form = this._formBuilder.group({
      id: [element.id],
      tipo: ['Autorizacion'],
      dependenciaAutorizadorId: [null, [Validators.required]],
    ...
    this._service.getCentroOperativos().then(resp => {
      this.coAutorizadorOptions = resp;
    });
  }

  ngOnInit(): void {

    this.selectCOAutorizador();
    this.selectCODependiente();

    if (this.id != undefined) {
      this._service.getAplicacionExternaCargos(this.element.id).then(resp => {
        this.item = resp;
        ...
        this.form.patchValue({
          id: this.item.id,
          dependenciaAutorizadorId: this.item.cargoDependenciaIndependiente.dependenciaId,
          centroOperativoDependienteId: this.item.centroOperativoDependienteId,


        });

        this._service.getCargos(this.item.cargoDependenciaIndependiente.cargo.codigo, this._dependenciaAutorizadorId.value).then(resp => {
          this.form.patchValue({
            cargoIndependienteId: resp[0]
          });
          this.centroOperativoAutorizador(this.item.cargoDependenciaIndependiente.cargo.clase);
        })
    ...
    this.form.get('dependenciaAutorizadorId').valueChanges.subscribe(
      (valueDependencia) => {
        ...
        if (valueDependencia != null && valueDependencia != undefined) {
          this.filteredCargos = this.form.get('cargoIndependienteId')
            ...
            .pipe(
              debounceTime(300),
              switchMap(value => this._service.getCargos(value, this._dependenciaAutorizadorId.value))
            )
        }
      }
    );
    ...
    this.form.get('dependenciaAutorizadorId').valueChanges.subscribe(
      (value) => {
        if (value != null && value != undefined) {
          this._cargoAutorizadores(value, this._dependenciaAutorizadorId.value);
        }
      }
    );
    ...
    this.form.get('cargoIndependienteId').valueChanges.subscribe(
      (value) => {
        if(value.cargo) {
          this.centroOperativoAutorizador(value.cargo.clase);
  ...
  centroOperativoAutorizador(clase: string): void {
    switch (clase) {
  ...
  public selectCOAutorizador(): void {
  ...
  private _cargoAutorizadores(filtro, id): void {
    if (id != undefined) {
  ...
  get _dependenciaAutorizadorId() { return this.form.get('dependenciaAutorizadorId') }
