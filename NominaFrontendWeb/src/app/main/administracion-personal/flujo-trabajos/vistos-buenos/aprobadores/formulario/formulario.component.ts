...
import { AprobadoresFormularioService } from './formulario.service';
...
@Component({
  selector: 'aprobaciones-aprobadores-formulario',
  templateUrl: './formulario.component.html',
  styleUrls: ['./formulario.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class AprobadoresFormularioComponent implements OnInit {
  ...
  coAprobadorOptions: any;
  claseAprobador = claseAprobador;
  cargoAprobadorOptions: any[];
  ...
  constructor(
    public dialogRef: MatDialogRef<AprobadoresFormularioComponent>,
    @Inject(MAT_DIALOG_DATA) public element: any,
    private _formBuilder: FormBuilder,
    private _alcanosSnackBar: AlcanosSnackBarService,
    private _service: AprobadoresFormularioService,
    ...
    this.form = this._formBuilder.group({
      id: [element.id],
      tipo: ['Aprobacion'],
      dependenciaAprobadorId: [null, [Validators.required]],
    ...
    this._service.getCentroOperativos().then(resp => {
      this.coAprobadorOptions = resp;
    });
  }

  ngOnInit(): void {

    this.selectCOAprobador();
    ...
    if (this.id != undefined) {
      this._service.getAplicacionExternaCargos(this.element.id).then(resp => {
        ...
        this.form.patchValue({
          id: this.item.id,
          dependenciaAprobadorId: this.item.cargoDependenciaIndependiente.dependenciaId,
          centroOperativoDependienteId: this.item.centroOperativoDependienteId,


        });

        this._service.getCargos(this.item.cargoDependenciaIndependiente.cargo.codigo, this._dependenciaAprobadorId.value).then(resp => {
          this.form.patchValue({
            cargoIndependienteId: resp[0]
          });
          this.centroOperativoAprobador(this.item.cargoDependenciaIndependiente.cargo.clase);
        })
    ...
    this.form.get('dependenciaAprobadorId').valueChanges.subscribe(
      (valueDependencia) => {
        ...
        if (valueDependencia != null && valueDependencia != undefined) {
          this.filteredCargos = this.form.get('cargoIndependienteId')
            ...
            .pipe(
              debounceTime(300),
              switchMap(value => this._service.getCargos(value, this._dependenciaAprobadorId.value))
            )
        }
      }
    );
    ...
    this.form.get('dependenciaAprobadorId').valueChanges.subscribe(
      (value) => {
        if (value != null && value != undefined) {
          this._cargoAprobadores(value, this._dependenciaAprobadorId.value);
        }
      }
    );
    ...
    this.form.get('cargoIndependienteId').valueChanges.subscribe(
      (value) => {
        if(value.cargo) {
          this.centroOperativoAprobador(value.cargo.clase);
  ...
  centroOperativoAprobador(clase: string): void {
    switch (clase) {
  ...
  private _cargoAprobadores(filtro, id): void {
    if (id != undefined) {
  ...
  get _dependenciaAprobadorId() { return this.form.get('dependenciaAprobadorId') }
