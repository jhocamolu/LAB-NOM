...
export class AprobadoresListarService {
  ...
  public buildFilter(filters): Promise<any> {
    ...
    if (this.urlFilters.hasOwnProperty('$filter')) {
      ...
      urlFilters.push(`${decodeURIComponent(`tipo`)} eq 'Aprobacion'`);
      // urlFilters.push(`${decodeURIComponent(`tipo`)} eq ${decodeURIComponent("Aprobacion")}`);
    ...
    return this._getAprobadores(this.urlFilters);

  }

  public getAprobadores(): void{
    this._getAprobadores(this.urlFilters); 
  }



  private _getAprobadores(params: any): Promise<any> {

    let filter = '';
    if (!params.hasOwnProperty('$filter')) {
      filter = `$filter= tipo eq 'Aprobacion' and aplicacionExternaId eq ${this.id}`;
    }

    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    // const paramsAdd = encodeURI(`$filter=tipo eq 'Aprobacion'`);
    const expand = encodeURI('$expand=centroOperativoDependiente($select=nombre),cargoDependenciaIndependiente($expand=cargo,dependencia),centroOperativoIndependiente($select=nombre)');
  ...
  public getCargosList(): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.get(`${environmentAlcanos.configuracionGeneral}/odata/cargos?$select=id,nombre&$filter=estadoRegistro eq 'Activo'`)
        .subscribe((response: any) => {
