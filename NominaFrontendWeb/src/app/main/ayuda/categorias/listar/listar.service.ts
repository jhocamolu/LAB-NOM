...
export class ListarService implements Resolve<any>{
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
    ...
    if (!this.urlFilters.hasOwnProperty('$orderBy')) {
      this.urlFilters.$orderBy = 'padre/nombre asc';
    }

    return new Promise((resolve, reject) => {
      Promise.all([
        this._getCategorias(this.urlFilters)
      ]).then(
        () => {
          resolve();
        },
   ...
   * @returns {Promise<any>}
   */
  private _getCategorias(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    const url = `${environmentAlcanos.ayuda}/odata/categorias?$expand=padre&$count=true&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.ayuda}/api/categorias/${id}`, {
        id: id,
        activo: activo
      }).subscribe((response: any) => {
        resolve(response);
