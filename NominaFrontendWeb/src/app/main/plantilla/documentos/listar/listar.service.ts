...
export class ListarService implements Resolve<any>{
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
    ...
    if (!this.urlFilters.hasOwnProperty('$orderBy')) {
      this.urlFilters.$orderBy = 'fechaVigencia desc';
    }

    return new Promise((resolve, reject) => {
      Promise.all([
        this._getDocumentos(this.urlFilters),
        this._getGrupos()
      ]).then(
        () => {
          resolve();
   ...
   * @returns {Promise<any>}
   */
  private _getDocumentos(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    const url = `${environmentAlcanos.plantillas}/odata/plantillas?$count=true&$expand=grupoDocumento&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.plantillas}/api/plantillas/${id}`, {
        id: id,
        activo: activo
      }).subscribe((response: any) => {
        resolve(response);
