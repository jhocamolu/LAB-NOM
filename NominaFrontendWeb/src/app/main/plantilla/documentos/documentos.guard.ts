...
export class DocumentosGuard implements CanActivate {
  ...
  check(url: string): boolean {
    ...
    if (url.includes('/plantilla/documentos')) {
      if (url.includes('/crear')) {
        if (this.permisos.find(x => x === 'Plantillas_Crear')) {
          return true;
        }
      }else  if (url.includes('/editar')) {
        if (this.permisos.find(x => x === 'Plantillas_Actualizar')) {
          return true;
        }
      }else if (this.permisos.find(x => x === 'Plantillas_Listar')) {
        return true;
      }
    }
