...
import { CrearBeneficioCorporativoService } from './crear.service';
...
@Component({
    selector: 'beneficio-corporativo-crear',
    templateUrl: './crear.component.html',
    styleUrls: ['./crear.component.scss'],
    animations: fuseAnimations,
    encapsulation: ViewEncapsulation.None
})
export class CrearBeneficioCorporativoComponent implements OnInit {
    form: FormGroup;
    submit: boolean;
    getCentroOperativoOptions: any[] = [];
    getDependenciaOptions: any[] = [];
    tipoBeneficiosOptions: any[];
    ...
    constructor(
        ...
        private _service: CrearBeneficioCorporativoService
    ) {

        this.tipoBeneficiosOptions = [];
    ...
    ngOnInit(): void {
        ...
        this._service.getTipoBeneficios().then(resp => {
            this.tipoBeneficiosOptions = resp;
        });
    }
    ...
    get tipoBeneficio(): AbstractControl {
        return this.form.get('tipoBeneficio');
    }
    ...
    guardarHandle(event): void {
        ...
        if (formValue.tipoBeneficio != null) {
            if (typeof formValue.tipoBeneficio != 'string') {
                if (formValue.tipoBeneficio == "") {
                    formValue.tipoBeneficio = null;
                } else {
                    formValue.tipoBeneficio = formValue.tipoBeneficio.join(',');
                }
            }
        }
        ...
        this._service.crear(formValue)
            .then((resp) => {
                ...
                if (resp.status === 400) {
                    ...
                    if ('tipoBeneficio' in error) {
                        const errores = {};
                        error.tipoBeneficio.forEach(element => {
                            errores[element] = true;
                        });
                        this.tipoBeneficio.setErrors(errores);
                    }
