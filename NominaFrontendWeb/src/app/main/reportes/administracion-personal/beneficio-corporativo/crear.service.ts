...
export class CrearBeneficioCorporativoService {
    ...
    public getTipoBeneficios(): Promise<any> {
        const params = encodeURI(`$select=id,nombre&$orderby=nombre asc&$filter=estadoRegistro eq 'Activo'`);
        const url = `${environmentAlcanos.administracionPersonal}/odata/tipobeneficios?${params}`;
        return new Promise((resolve, reject) => {
            this._httpClient.get(url).subscribe((response: any) => {
                resolve(response.value);
            }, reject);
    ...
    crear(dato: any): Promise<any> {
        return new Promise((resolve, reject) => {
            this._httpClient.post(`${environmentAlcanos.configuracionGeneral}/reporte/BeneficioCorporativo`, dato)
                .subscribe((response: any) => {
                    resolve(response);
                }, reject);
        });
