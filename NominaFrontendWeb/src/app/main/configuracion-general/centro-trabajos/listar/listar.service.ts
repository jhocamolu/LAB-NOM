...
export class ListarService implements Resolve<any>{
  ...
  public getCentroTrabajos(params: any): Promise<any> {
    ...
    const url = `${environmentAlcanos.configuracionGeneral}/odata/centroTrabajos?$count=true&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.configuracionGeneral}/api/centroTrabajos/${id}`, {
        id: id,
        activo: activo
      })
        .subscribe((response: any) => {
