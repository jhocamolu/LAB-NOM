...
@Component({
  selector: 'centro-trabajos-listar',
...
export class ListarComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  displayedColumns: string[] = ['codigo', 'nombre', 'porcentajeRiesgo', 'estadoRegistro', 'acciones'];
  ...
  constructor(
    ...
    this.arrayPermisos = this._permisos.permisosStorage('CentroTrabajos_');
  ...
  crearHandle(event): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this._service.getCentroTrabajos(this._service.urlFilters);
      }
    });
  }
