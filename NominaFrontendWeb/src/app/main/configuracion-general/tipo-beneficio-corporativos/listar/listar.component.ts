...
export class ListarComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  displayedColumns: string[] = ['nombre', 'conceptoNominaDevengoId', 'estadoRegistro', 'acciones'];
    ...
    this.paginator.page.subscribe(
      (event: PageEvent) => {
        ...
        this._router.navigate(
          ['/administracion-personal/tipo-beneficios/'],
          {
            queryParams: {
              $top: event.pageSize,
              $skip: skip,
  ...
  crearHandle(event): void {
    this._router.navigate(
      ['/configuracion/beneficios/crear'],
    );
  }
    ...
    dialogRef.afterClosed().subscribe(confirm => {
      if (confirm) {
        ...
        this._service.activo(id, bool).then(result => {
          this._service.getTipoBeneficios();
