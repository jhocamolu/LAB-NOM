...
export class ListarComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  constructor(
    ...
    this.arrayPermisos = this._permisos.permisosStorage('GrupoNominas_');
  ...
  crearHandle(event): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this._service.getGrupoNominas(this._service.urlFilters);
      }
    });
  }
