...
export class CargosGuard implements CanActivate {
  ...
  check(url: string): boolean {
    if (url.includes('/configuracion/cargos')) {
      if (this.permisos.find(x => x === 'Cargos_Listar')) {
        return true;
      }
    }

    if (url.includes('/mostrar')) {
      if (this.permisos.find(x => x === 'Cargos_Obtener')) {
        return true;
      }
    }

    if (url.includes('/editar')) {
      if (this.permisos.find(x => x === 'Cargos_Actualizar')) {
        return true;
      }
    }
