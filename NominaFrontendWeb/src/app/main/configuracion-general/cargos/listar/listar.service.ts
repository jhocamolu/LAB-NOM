...
export class ListarService implements Resolve<any>{
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
    ...
    if (this.urlFilters.hasOwnProperty('$filter')) {
      ...
      this.urlFilters['$filter'].replace(/([^=&]+)=([^&]*)/g, function (m, key, value) {
        ...
        if (decodeURIComponent(value) !== 'null') {
          ...
          }else if (decodeURIComponent(value) !== 'null') {
            dataFilters[decodeURIComponent(key)] = decodeURIComponent(value);
    ...
    return new Promise((resolve, reject) => {
      Promise.all([
        this._getCargos(this.urlFilters)
      ]).then(
        () => {
          resolve();
        },
   ...
   * @returns {Promise<any>}
   */
  private _getCargos(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    // V3 estoyReportandoA($expand=cargoJefe)
    const url = `${environmentAlcanos.administracionPersonal}/odata/cargos?$expand=nivelcargo&$count=true&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.administracionPersonal}/api/cargos/${id}`, {
        id: id,
        activo: activo
      })
        .subscribe((response: any) => {
