...
export class ListarService implements Resolve<any>{
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
        ...
        if (decodeURIComponent(value) !== 'null') {
          switch (decodeURIComponent(key)) {
            case 'pagoPorDefecto':
              ...
              urlFilters.push(`pagoPorDefecto eq ${decodeURIComponent(value)}`);
              break;
    ...
    return new Promise((resolve, reject) => {
      Promise.all([
        this._getTipoPeriodos(this.urlFilters)
      ]).then(
        () => {
          resolve();
        },
   ...
   * @returns {Promise<any>}
   */
  private _getTipoPeriodos(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    const url = `${environmentAlcanos.configuracionGeneral}/odata/TipoPeriodos?$count=true&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.configuracionGeneral}/api/TipoPeriodos/${id}`, {
        id: id,
        activo: activo
      })
        .subscribe((response: any) => {
