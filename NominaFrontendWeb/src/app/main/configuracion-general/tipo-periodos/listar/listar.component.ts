...
export class ListarComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  displayedColumns: string[] = ['nombre', 'pagoPorDefecto', 'estadoRegistro', 'acciones'];
  ...
  urlLink(event): any {
    return this._router.navigate(['/configuracion/tipo-periodos/crear']);
  }
  ...
  mostrarHandle(event, element): void {
    const dialogRef = this._matDialog.open(MostrarComponent, {
      panelClass: 'mostrar-dialog-tipo-periodos',
      disableClose: false,
      data: element
    });
    dialogRef.afterClosed().subscribe(result => {
