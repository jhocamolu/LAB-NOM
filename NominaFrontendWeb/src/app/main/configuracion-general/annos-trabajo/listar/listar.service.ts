...
export class ListarService implements Resolve<any>{
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
    ...
    if (!this.urlFilters.hasOwnProperty('$orderBy')) {
      this.urlFilters.$orderBy = 'anno desc';
    }

    return new Promise((resolve, reject) => {
      Promise.all([
        this.getAnnosTrabajo(this.urlFilters)
      ]).then(
        () => {
          resolve();
        },
   ...
   * @returns {Promise<any>}
   */
  public getAnnosTrabajo(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    const url = `${environmentAlcanos.configuracionGeneral}/odata/AnnoVigencias?$select=id,anno,estado,fechaCreacion&$count=true&${toUrlEncoded(params)}`;
   ...
   * @returns {Promise<any>}
   */
  estado(id: number, activo: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.configuracionGeneral}/api/AnnoVigencias/${id}`, {
        id: id,
        estado: activo
      })
        .subscribe((response: any) => {
