...
export class ConceptoService {
  ...
  public buildFilter(filters): Promise<any> {
    ...
    if (filters.hasOwnProperty('$orderBy')) {
      ...
      if (this.orderFilters['$orderBy'] === null) {
        this.orderFilters['$orderBy'] = 'conceptoNomina/nombre asc';
      }

    }

    return this._getConceptos(this.id, this.paginadorFilters, this.orderFilters);
   ...
   * @id Id del concepto
   * @returns {Promise<any>}
   * Obtiene Todos los conceptos para llenar select asc Nombre
   */
  private _getConceptos(id: number, paginador: any, order: any | null): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    const filter = encodeURI(`$filter=tipoAusentismoId eq ${id}`);
    const expand = encodeURI('$expand=conceptoNomina');
    let urlOrder = '';
    if (order !== null) {
      urlOrder = toUrlEncoded(order);
    }
    const url = `${environmentAlcanos.nomina}/odata/TipoAusentismoConceptoNominas?$count=true&${expand}&${filter}&${toUrlEncoded(paginador)}&${urlOrder}`;
  ...
  eliminarHandle(id: number): Promise<any> {
    const url = `${environmentAlcanos.nomina}/api/TipoAusentismoConceptoNominas/${id}`;
    return new Promise((resolve, reject) => {
