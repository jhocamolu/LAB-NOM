...
export class ListarService implements Resolve<any> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
    ...
    if (this.urlFilters.hasOwnProperty('$orderBy')) {
      if ( this.urlFilters.$orderBy === 'clase asc') {
        this.urlFilters.$orderBy = 'claseAusentismo/nombre asc';
      }
      if (this.urlFilters.$orderBy === 'clase desc') {
        this.urlFilters.$orderBy = 'claseAusentismo/nombre desc';
      }
    ...
    return new Promise((resolve, reject) => {
      Promise.all([
        this._getTipoAusentismos(this.urlFilters)
      ]).then(
        () => {
          resolve();
        },
  ...
  public getTipoAusentismos(): void {
    this._getTipoAusentismos(this.urlFilters);
  }
  /**
   * 
   ...
   * @returns {Promise<any>}
   */
  private _getTipoAusentismos(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    const url = `${environmentAlcanos.configuracionGeneral}/odata/TipoAusentismos?$expand=claseAusentismo&$count=true&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.configuracionGeneral}/api/TipoAusentismos/${id}`, {
        id: id,
        activo: activo
      })
        .subscribe((response: any) => {
