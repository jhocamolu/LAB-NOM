...
export class ListarService implements Resolve<any>{
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
    ...
    return new Promise((resolve, reject) => {
      Promise.all([
        this._getJornadaLaborales(this.urlFilters)
      ]).then(
        () => {
          resolve();
        },
  ...
  public getJornadaLaborales(){
    return this._getJornadaLaborales(this.urlFilters);
  }
   ...
   * @returns {Promise<any>}
   */
  public _getJornadaLaborales(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    const otherParams = `$expand=jornadaLaboralDias`; 
    const url = `${environmentAlcanos.configuracionGeneral}/odata/JornadaLaborales?$count=true&${toUrlEncoded(params)}&${otherParams}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.configuracionGeneral}/api/JornadaLaborales/${id}`, {
        id: id,
        activo: activo
      })
        .subscribe((response: any) => {
