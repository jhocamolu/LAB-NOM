...
@Component({
  selector: 'jornada-laborales-listar',
...
export class ListarComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  displayedColumns: string[] = ['nombre', 'jornadaLaboralDias', 'estadoRegistro', 'acciones'];
  ...
  constructor(
    ...
    this.arrayPermisos = this._permisos.permisosStorage('JornadaLaborales_');
    this.dataRequest = false;
  }

  ngOnInit(): void {
    this._service.dataRequest.subscribe((resp: boolean) => {
    ...
    this.paginator.page.subscribe((event: PageEvent) => {
      ...
      this._router.navigate(['/configuracion/jornada-laborales'], {
        queryParams: {
          $top: event.pageSize,
          $skip: skip
        },
  ...
  activarHandle(event, element): void {
    ...
    const verboInverso =
      element.estadoRegistro === 'Activo' ? 'inactivarlo' : 'activarlo';
    ...
    dialogRef.afterClosed().subscribe(confirm => {
      if (confirm) {
        ...
        this._service.activo(id, bool).then(result => {
          this._service.getJornadaLaborales();
  ...
  urlLink(event): any {
    return this._router.navigate(['/configuracion/jornada-laborales/crear']);
  }
  ...
  crearHandle(event): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this._service._getJornadaLaborales(this._service.urlFilters);
      }
    });
  }
