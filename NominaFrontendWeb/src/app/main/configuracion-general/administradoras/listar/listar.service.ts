...
export class ListarService implements Resolve<any>{
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
    ...
    return new Promise((resolve, reject) => {
      Promise.all([
        this._getAdministradoras(this.urlFilters)
      ]).then(
        () => {
          resolve();
        },
  ...
  public getAdministradoras(): any{
    return this._getAdministradoras(this.urlFilters);
  }
  /**
   * 
   *
   * @returns {Promise<any>}
   */
  public _getAdministradoras(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    const url = `${environmentAlcanos.configuracionGeneral}/odata/administradoras?$expand=tipoAdministradora&$count=true&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.configuracionGeneral}/api/administradoras/${id}`, {
        id: id,
        activo: activo
      })
        .subscribe((response: any) => {
