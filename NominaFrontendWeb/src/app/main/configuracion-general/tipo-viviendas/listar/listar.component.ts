...
@Component({
  selector: 'tipo-viviendas-listar',
...
export class ListarComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  displayedColumns: string[] = ['nombre', 'estadoRegistro', 'acciones'];
  ...
  constructor(
    ...
    this.arrayPermisos = this._permisos.permisosStorage('TipoViviendas_');
  ...
  crearHandle(event): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this._service.getTipoViviendas(this._service.urlFilters);
      }
    });
  }
