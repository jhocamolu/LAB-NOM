...
export class ListarService implements Resolve<any>{
  ...
  private _getHoraExtras(params: any): Promise<any> {
    ...
    const url = `${environmentAlcanos.configuracionGeneral}/odata/tipohoraextras/?$expand=conceptoNomina&$count=true&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.configuracionGeneral}/api/tipohoraextras/${id}`, {
        id: id,
        activo: activo
      })
        .subscribe((response: any) => {
