...
@Component({
  selector: 'tipo-documentos-listar',
...
export class ListarComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  displayedColumns: string[] = ['codigoPila', 'codigoDian', 'nombre', 'formato', 'equivalenteBancario', 'estadoRegistro', 'acciones'];
  ...
  constructor(
    ...
    this.arrayPermisos = this._permisos.permisosStorage('TipoDocumentos_');
  ...
  crearHandle(event): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this._service.getTipoDocumentos(this._service.urlFilters);
      }
    });
  }
