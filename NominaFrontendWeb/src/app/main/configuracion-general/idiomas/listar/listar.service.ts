...
export class ListarService implements Resolve<any>{
  ...
  public getIdiomas(params: any): Promise<any> {
    ...
    const url = `${environmentAlcanos.configuracionGeneral}/odata/idiomas?$count=true&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.configuracionGeneral}/api/idiomas/${id}`, {
        id: id,
        activo: activo
      })
        .subscribe((response: any) => {
