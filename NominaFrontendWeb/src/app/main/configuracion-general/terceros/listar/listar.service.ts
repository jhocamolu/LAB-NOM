...
export class ListarService implements Resolve<any> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
            ...
            this.urlFilters['$filter'].replace(/([^=&]+)=([^&]*)/g, function (m, key, value) {
                if (decodeURIComponent(value) !== 'null') {
                    ...
                    if (key === 'nit') {
                        ...
                        urlFilters.push(`contains(cast(${decodeURIComponent(`nit`)}, 'Edm.String'),'${decodeURIComponent(value)}')`);
                    }
                    
                    if (key === 'nombre') {
                        dataFilters[decodeURIComponent(key)] = decodeURIComponent(value);
                        urlFilters.push(`contains(cast(${decodeURIComponent(`nombre`)}, 'Edm.String'),'${decodeURIComponent(value)}')`);
                    }
                }
            });
        ...
        return new Promise((resolve, reject) => {
            Promise.all([
                this._getTerceros(this.urlFilters)
            ]).then(
                () => {
                    resolve();
                },
     ...
     * @returns {Promise<any>}
     */
    private _getTerceros(params: any): Promise<any> {
        const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
        // tslint:disable-next-line: max-line-length
        
        const url = `${environmentAlcanos.novedades}/odata/terceros?$count=true&${toUrlEncoded(params)}`;
    ...
    activo(id: number, activo: boolean): Promise<any> {
        return new Promise((resolve, reject) => {
            this._httpClient.patch(`${environmentAlcanos.novedades}/api/terceros/${id}`, {
                id: id,
                activo: activo
            })
                .subscribe((response: any) => {
