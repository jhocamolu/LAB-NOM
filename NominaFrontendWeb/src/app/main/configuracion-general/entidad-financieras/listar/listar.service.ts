...
export class ListarService implements Resolve<any> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
      ...
      this.urlFilters['$filter'].replace(/([^=&]+)=([^&]*)/g, function (m, key, value) {
        ...
        if (decodeURIComponent(value) !== 'null') {
          switch (decodeURIComponent(key)) {
            // case 'numeroDocumentoFuncionario':
    ...
    return new Promise((resolve, reject) => {
      Promise.all([
        this._getEntidadFinancieras(this.urlFilters)
      ]).then(
        () => {
          resolve();
        },
  ...
  public getEntidadFinancieras(): void {
    this._getEntidadFinancieras(this.urlFilters);
  }
  /**
   * 
   ...
   * @returns {Promise<any>}
   */
  private _getEntidadFinancieras(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    // tslint:disable-next-line: max-line-length
    const url = `${environmentAlcanos.configuracionGeneral}/odata/entidadFinancieras?$expand=divisionPoliticaNivel2($expand=divisionPoliticaNivel1)&$count=true&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.nomina}/api/entidadFinancieras/${id}`, {
        id: id,
        activo: activo
      })
        .subscribe((response: any) => {
