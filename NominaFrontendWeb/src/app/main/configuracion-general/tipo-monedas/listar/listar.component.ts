...
export class ListarComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  displayedColumns: string[] = ['codigo', 'nombre', 'estadoRegistro', 'acciones'];
  ...
  constructor(
    ...
    this.arrayPermisos = this._permisos.permisosStorage('TipoMonedas_');
  ...
  crearHandle(event): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this._service.getTipoMonedas(this._service.urlFilters);
      }
    });
  }
