...
@Component({
  selector: 'nivel-educativos-listar',
...
export class ListarComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  displayedColumns: string[] = ['nombre', 'orden', 'estadoRegistro', 'acciones'];
  ...
  constructor(
    ...
    this.arrayPermisos = this._permisos.permisosStorage('NivelEducativos_');
    ...
    this.paginator.page.subscribe(
      (event: PageEvent) => {
        ...
        this._router.navigate(
          ['/configuracion/nivel-educativos'],
          {
            queryParams: {
              $top: event.pageSize,
              $skip: skip,
  ...
  crearHandle(event): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this._service.getNivelEducativos(this._service.urlFilters);
      }
    });
  }
