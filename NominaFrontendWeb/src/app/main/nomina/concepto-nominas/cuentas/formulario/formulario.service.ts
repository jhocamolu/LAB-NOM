...
export class FormularioService {
   ...
   * @returns {Promise<any>}
   */
  public getConceptoCuentas(): Promise<any> {
    const params = encodeURI(`$orderby=nombre`);
    const url = `${environmentAlcanos.nomina}/odata/ConceptoNominaCuentaContables?${params}`;
    return new Promise((resolve, reject) => {
      this._httpClient.get(url).subscribe((response: any) => {
        resolve(response);
      }, reject);
  ...
  private _crear(dato: any): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.post(`${environmentAlcanos.configuracionGeneral}/api/ConceptoNominaCuentaContables`, dato)
  ...
  private _editar(dato: any): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.put(`${environmentAlcanos.configuracionGeneral}/api/ConceptoNominaCuentaContables/${dato.id}`, dato)
        .subscribe((response: any) => {
          resolve(response);
        }, reject);
    });
