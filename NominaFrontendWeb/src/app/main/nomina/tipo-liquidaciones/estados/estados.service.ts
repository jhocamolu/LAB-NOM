...
export class EstadosService {
  ...
  private _getEstados(id: number, paginador: any, order: any | null): Promise<any> {
    ...
    const filter = encodeURI(`$filter=tipoliquidacionId eq ${id}`);
    ...
    const url = `${environmentAlcanos.nomina}/odata/tipoliquidacionestados?$count=true&${filter}&${toUrlEncoded(paginador)}&${urlOrder}`;
  ...
  eliminarHandle(id: number): Promise<any> {
    const url = `${environmentAlcanos.nomina}/api/tipoliquidacionestados/${id}`;
    return new Promise((resolve, reject) => {
