...
export class MostrarService implements Resolve<any>{
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
    ...
    return new Promise((resolve, reject) => {
      Promise.all([
        this.getTipoLiquidaciones(this.id),
      ]).then(
        () => {
          resolve();
        },
   ...
   * @returns {Promise<any>}
   */
  private getTipoLiquidaciones(id: number): Promise<any> {
    const params = encodeURI('$expand=conceptoNominaAgrupador($select=id,nombre,alias,codigo,conceptoAgrupador),tipoPeriodo');
    const url = `${environmentAlcanos.configuracionGeneral}/odata/tipoliquidaciones/${id}?${params}`;
    return new Promise((resolve, reject) => {
  ...
  getTipoliquidacionModulos(id: number): Promise<any> {
    const params = encodeURI(`$filter=tipoLiquidacionId eq ${id}&$select=id,tipoLiquidacionId,modulo&$count=true`);
    return new Promise((resolve, reject) => {
