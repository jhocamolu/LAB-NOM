...
export class ListarService implements Resolve<any> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
    ...
    if (!this.urlFilters.hasOwnProperty('$orderBy')) {
      this.urlFilters.$orderBy = 'codigo asc';
    }
    return new Promise((resolve, reject) => {
      Promise.all([
        this._getDistribucionCostos(this.urlFilters)
      ]).then(
        () => {
          resolve();
        },
  ...
  public getDistribucionCostos(): void {
    this._getDistribucionCostos(this.urlFilters);
  }
   ...
   * @returns {Promise<any>}
   */
  private _getDistribucionCostos(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    const url = `${environmentAlcanos.configuracionGeneral}/odata/actividades?&$count=true&${toUrlEncoded(params)}`;
  ...
  activo(id: number, activo: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.patch(`${environmentAlcanos.configuracionGeneral}/api/actividades/${id}`, {
        id: id,
        activo: activo
      })
        .subscribe((response: any) => {
