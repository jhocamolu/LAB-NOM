...
export class ListarService implements Resolve<any> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
    ...
    return new Promise((resolve, reject) => {
      Promise.all([
        this._getTareaProgramadas(this.urlFilters)
      ]).then(
        () => {
          resolve();
        },
   ...
   * @returns {Promise<any>}
   */
  private _getTareaProgramadas(params: any): Promise<any> {
    const toUrlEncoded = obj => Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
    const url = `${environmentAlcanos.configuracionGeneral}/odata/TareaProgramadas?$count=true&${toUrlEncoded(params)}`;
   ...
   * @returns {Promise<any>}
   */
  ejecutar(alias: number): Promise<any> {
    return new Promise((resolve, reject) => {
      this._httpClient.post(`${environmentAlcanos.nomina}/api/tareaProgramadas/${alias}/Ejecutar`, {})
        .subscribe((response: any) => {
          resolve(response);
        }, reject);
    });
