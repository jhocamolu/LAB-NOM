...
{
    public class CrearCandidatoHandler :Peticion, IRequestHandler<CrearCandidatoRequest, CommandResult>
    {
        private readonly IHttpContextAccessor httpContextAccessor;
        private readonly UserManager<UsuarioAplicacion> userManager;
        private readonly ReclutamientoDbContext contexto;

        public CrearCandidatoHandler(IConfiguration configuration, IPeticionService peticion, IHttpContextAccessor httpContextAccessor, UserManager<UsuarioAplicacion> userManager, ReclutamientoDbContext contexto)
        {
        ...
        public async Task<CommandResult> Handle(CrearCandidatoRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (tokenPortal != "")
                {
                    ...
                    var list = await this.EnviarServicios($"{host}/api/candidatos", request, usuario.TokenGhestic);
