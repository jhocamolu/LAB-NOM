...
{
    public class ParcialHojaDeVidaHandler : Peticion,IRequestHandler<ParcialHojaDeVidaRequest, CommandResult>
    {
        private readonly IHttpContextAccessor httpContextAccessor;
        private readonly UserManager<UsuarioAplicacion> userManager;
        private readonly ReclutamientoDbContext contexto;

        public ParcialHojaDeVidaHandler(IConfiguration configuration, IPeticionService peticion,IHttpContextAccessor httpContextAccessor, UserManager<UsuarioAplicacion> userManager, ReclutamientoDbContext contexto)
        {
            ...
            try
            {
               var tokenPortal = InformacionToken.ObtenerTokenUsuario(httpContextAccessor);
                if (tokenPortal != "")
                {
                    ...
                    var list = await this.ParcialServicios($"{host}/api/HojaDeVidas/" + request.Id, request, usuario.TokenGhestic);
