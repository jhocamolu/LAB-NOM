...
{
    public class CrearHojaDeVidaDocumentoHandler : Peticion, IRequestHandler<CrearHojaDeVidaDocumentoRequest, CommandResult>
    {
        private readonly IHttpContextAccessor httpContextAccessor;
        private readonly UserManager<UsuarioAplicacion> userManager;
        private readonly ReclutamientoDbContext contexto;

        public CrearHojaDeVidaDocumentoHandler(IConfiguration configuration, IPeticionService peticion, IHttpContextAccessor httpContextAccessor, UserManager<UsuarioAplicacion> userManager, ReclutamientoDbContext contexto)
        {
        ...
        public async Task<CommandResult> Handle(CrearHojaDeVidaDocumentoRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (tokenPortal != "")
                {
                    ...
                    var list = await this.EnviarServicios($"{host}/api/hojadevidadocumentos", request, usuario.TokenGhestic);
