...
{
    public class ActualizarHojaDeVidaDocumentoHandler : Peticion, IRequestHandler<ActualizarHojaDeVidaDocumentoRequest, CommandResult>
    {
        private readonly IHttpContextAccessor httpContextAccessor;
        private readonly UserManager<UsuarioAplicacion> userManager;
        private readonly ReclutamientoDbContext contexto;

        public ActualizarHojaDeVidaDocumentoHandler(IConfiguration configuration, IPeticionService peticion, IHttpContextAccessor httpContextAccessor, UserManager<UsuarioAplicacion> userManager, ReclutamientoDbContext contexto)
        {
        ...
        public async Task<CommandResult> Handle(ActualizarHojaDeVidaDocumentoRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (tokenPortal != "")
                {
                    ...
                    var list = await this.ActualizarServicios($"{host}/api/HojaDeVidaDocumentos/" + request.Id, request, usuario.TokenGhestic);
