...
{
    public class ActualizarHojaDeVidaExperienciaLaboralHandler : Peticion, IRequestHandler<ActualizarHojaDeVidaExperienciaLaboralRequest, CommandResult>
    {
        private readonly IHttpContextAccessor httpContextAccessor;
        private readonly UserManager<UsuarioAplicacion> userManager;
        private readonly ReclutamientoDbContext contexto;

        public ActualizarHojaDeVidaExperienciaLaboralHandler(IConfiguration configuration, IPeticionService peticion, IHttpContextAccessor httpContextAccessor, UserManager<UsuarioAplicacion> userManager, ReclutamientoDbContext contexto)
        {
        ...
        public async Task<CommandResult> Handle(ActualizarHojaDeVidaExperienciaLaboralRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (tokenPortal != "")
                {
                    ...
                    var list = await this.ActualizarServicios($"{host}/api/HojaDeVidaExperienciaLaborales/" + request.Id, request, usuario.TokenGhestic);
