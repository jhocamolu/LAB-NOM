...
namespace Reclutamiento.Dominio.HojaDeVidaEstudios.Comandos.Actualizar
{
    public class ActualizarHojaDeVidaEstudioHandler : Peticion, IRequestHandler<ActualizarHojaDeVidaEstudioRequest, CommandResult>
    {
        ...
        public ActualizarHojaDeVidaEstudioHandler(IConfiguration configuration, IPeticionService peticion, IHttpContextAccessor httpContextAccessor, UserManager<UsuarioAplicacion> userManager, ReclutamientoDbContext contexto)
        {
        ...
        public async Task<CommandResult> Handle(ActualizarHojaDeVidaEstudioRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (tokenPortal != "")
                {
                    ...
                    var list = await this.ActualizarServicios($"{host}/api/HojaDeVidaEstudios/" + request.Id, request, usuario.TokenGhestic);
