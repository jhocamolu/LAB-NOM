...
{
    public class EliminarHojaDeVidaEstudioHandler : Peticion, IRequestHandler<EliminarHojaDeVidaEstudioRequest, CommandResult>
    {
        private readonly IHttpContextAccessor httpContextAccessor;
        private readonly UserManager<UsuarioAplicacion> userManager;
        private readonly ReclutamientoDbContext contexto;

        public EliminarHojaDeVidaEstudioHandler(IConfiguration configuration, IPeticionService peticion, IHttpContextAccessor httpContextAccessor, UserManager<UsuarioAplicacion> userManager, ReclutamientoDbContext contexto)
        {
        ...
        public async Task<CommandResult> Handle(EliminarHojaDeVidaEstudioRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (tokenPortal != "")
                {
                    ...
                    var consulta = this.contexto.HojaDeVidaEstudios.FirstOrDefault(x => x.Id == request.Id);
                    ...
                    var list = await this.EliminarServicios($"{host}/api/HojadevidaEstudios/" + request.Id, usuario.TokenGhestic);
