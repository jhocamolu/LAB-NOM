...
{
    public class ObtenerSueldoHandler : IRequestHandler<ObtenerSueldoRequest, CommandResult>
    {
        private readonly IPeticionService peticionService;
        private readonly IConfiguration configuration;
        private readonly NominaDbContext contexto;

        public ObtenerSueldoHandler(IPeticionService peticionService, IConfiguration configuration, NominaDbContext contexto)
        {
        ...
        public async Task<CommandResult> Handle(ObtenerSueldoRequest request, CancellationToken cancellationToken)
        {
            try
            {
                var ambiente = configuration.GetValue<string>(RutaArchivos.Ambiente()).Split(';')[1].Split('-')[1];
                string url = configuration.GetValue<string>(
                    RutaArchivos.UrlCertificado(ambiente, TipoCertificado.SUELDO));
