...
{
    public class ObtenerCargoHandler : IRequestHandler<ObtenerCargoRequest, CommandResult>
    {
        private readonly IPeticionService peticionService;
        private readonly IConfiguration configuration;
        private readonly NominaDbContext contexto;

        public ObtenerCargoHandler(IPeticionService peticionService, IConfiguration configuration, NominaDbContext contexto)
        {
        ...
        public async Task<CommandResult> Handle(ObtenerCargoRequest request, CancellationToken cancellationToken)
        {
            try
            {
                var ambiente = configuration.GetValue<string>(RutaArchivos.Ambiente()).Split(';')[1].Split('-')[1];
                string url = configuration.GetValue<string>(
                    RutaArchivos.UrlCertificado(ambiente, TipoCertificado.CARGO));
