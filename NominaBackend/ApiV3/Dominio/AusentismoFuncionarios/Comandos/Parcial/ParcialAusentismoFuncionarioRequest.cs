...
{
    public class ParcialAusentismoFuncionarioRequest : IRequest<CommandResult>, IValidatableObject
    {
        #region Validacion
        ...
        [RegularExpression(@"^[" + ConstantesExpresionesRegulares.Numerico + "]+$", ErrorMessage = ConstantesErrores.Alfanumerico)]
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            try
            {
                TipoAusentismo validaTipoAusentismo = null;
                Funcionario validaFuncionario = null;
                ...
                var validaFechaInicioEntre = dbContexto.AusentismoFuncionarios.FirstOrDefault(x =>
                ...
                if (validaFechaInicioEntre != null)
                {
                    errores.Add(new ValidationResult(
                         $"No se puede guardar el ausentismo ya que se encuentra vigente un ausentismo anterior en las fechas ingresadas.",
                         new[] { "Snack" }));
                }
                #endregion
                ...
                {
                    validaTipoAusentismo = dbContexto.TipoAusentismos.FirstOrDefault(x => x.Id == TipoAusentismoId);
                ...
                #region FuncionarioId
                    ...
                    validaFuncionario = dbContexto.Funcionarios.FirstOrDefault(x => x.Id == ausentismo.FuncionarioId);
                ...
                if (ProrrogaId != null)
                {
                    var validaProrrogaDe = dbContexto.ProrrogaAusentismos.FirstOrDefault(x => x.ProrrogaId == ProrrogaId
                                                                                            && x.AusentismoId != Id
                                                                                            && x.EstadoRegistro == EstadoRegistro.Activo);
                    if (validaProrrogaDe != null)
                    {
                        errores.Add(new ValidationResult(
                    ...
                    else
                    {
                        ...
                        var diaAnterior = (DateTime)FechaInicio.AddDays(-1);
                ...
                if (FechaFin != fechaVacia)
                {
                    ...
                    {
                        if (FechaFin < ausentismo.FechaInicio)
                        {
                            errores.Add(new ValidationResult("La fecha final que intentas guardar no puede ser menor que la fecha inicial.",
                                new[] { "FechaFin" }));
                        }
                ...
                if (validaTipoAusentismo != null)
                {
                    ...
                    if (claseAusentismo.Codigo == "LR" && validaTipoAusentismo.Codigo != "LPL" && validaFuncionario.Estado == EstadoFuncionario.EnVacaciones)
                    {
                        errores.Add(new ValidationResult(
                ...
                if (Aprobado != null)
                {
                    if (Aprobado == false && string.IsNullOrEmpty(Justificacion))
                    {
