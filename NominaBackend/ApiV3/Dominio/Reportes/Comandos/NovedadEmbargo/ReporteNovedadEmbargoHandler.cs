...
namespace ApiV3.Dominio.Reportes.Comandos.NovedadEmbargo
{
    public class ReporteNovedadEmbargoHandler : Peticion, IRequestHandler<ReporteNovedadEmbargoRequest, CommandResult>
    {
        const string ALIAS = "NovedadesEmbargo";

        public ReporteNovedadEmbargoHandler(IConfiguration configuration, IPeticionService peticion)
        {
            this.configuration = configuration;
            this.peticion = peticion;
        }

        public async Task<CommandResult> Handle(ReporteNovedadEmbargoRequest request, CancellationToken cancellationToken)
        {
            try
            {
