...
{
    public class ReporteProrrogaContratoTerminoFijoRequest : IRequest<CommandResult>, IValidatableObject
    {
        [Required(ErrorMessage = ConstantesErrores.Requerido)]
        public string TipoContratoTerminoFijo { get; set; }
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(TipoContratoTerminoFijo))
                {
                    var splitCentroOperativo = TipoContratoTerminoFijo.Split(',');
                    foreach (var item in splitCentroOperativo)
                    {
                        var validaTipoContrato = contexto.TipoContratos.FirstOrDefault(c => c.Id == int.Parse(item));
                        if (validaTipoContrato == null)
                        {
                            errores.Add(new ValidationResult(
                                $"No existe uno de los tipos de contrato que esta seleccionando.",
                                new[] { "TipoContrato" }));
                        }
                    }
                }
