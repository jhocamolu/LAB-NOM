...
{
    public class ReporteArchivoTipo1PilaHandler : IRequestHandler<ReporteArchivoTipo1PilaRequest, CommandResult>
    {
        private readonly NominaDbContext contexto;
        private IConfiguration configuration;
        private IPeticionService peticion;
        private readonly string host;
        const string ALIAS = "ArchivoTipo1Pila";

        public ReporteArchivoTipo1PilaHandler(IConfiguration configuration, IPeticionService peticion, NominaDbContext contexto)
        {
        ...
        public async Task<CommandResult> Handle(ReporteArchivoTipo1PilaRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                object parametros;
                string codigoTipoAcccion = null;
                if (request.TipoAccionId != null)
                {
                    codigoTipoAcccion = contexto.TipoAcciones.FirstOrDefault(x => x.Id == request.TipoAccionId).Codigo.ToString();
                }

                // parametros del reporte
                parametros = new
                {
                    TipoAccionCodigo = codigoTipoAcccion,
