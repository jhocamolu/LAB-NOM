...
{
    public class ReporteArchivoTipo2PilaHandler : IRequestHandler<ReporteArchivoTipo2PilaRequest, CommandResult>
    {
        private readonly NominaDbContext contexto;
        private IConfiguration configuration;
        private IPeticionService peticion;
        private readonly string host;
        const string ALIAS = "ArchivoTipo2Pila";

        public ReporteArchivoTipo2PilaHandler(IConfiguration configuration, IPeticionService peticion, NominaDbContext contexto)
        {
        ...
        public async Task<CommandResult> Handle(ReporteArchivoTipo2PilaRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                parametros = new
                {
                    PeriodoPagoId = request.PeriodoPagoId,
                    TipoPlanillaId = request.TipoPlanillaId,
                    NumeroPlanilla = request.NumeroPlanilla,
                    FechaPagoPlanilla = request.FechaPagoPlanilla,
                    TipoCotizanteId = request.TipoCotizanteId,
                    SubtiposCotizante = request.SubtipoCotizante,
                    Funcionarios = request.Funcionario,
