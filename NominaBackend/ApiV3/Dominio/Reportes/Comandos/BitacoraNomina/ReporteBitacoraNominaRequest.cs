...
{
    public class ReporteBitacoraNominaRequest : IRequest<CommandResult>, IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            try
            {
                ...
                if (Metodo == MetodoReporteEmbargo.PorPeriodo)
                {
                    ...
                    if (SubperiodoId != null)
                    {
                        var splitSubperiodo = SubperiodoId.Split(',');
                        foreach (var item in splitSubperiodo)
                        {
                            var validaSubperiodo = dbContexto.SubPeriodos.FirstOrDefault(c => c.Id == int.Parse(item));
                            if (validaSubperiodo == null)
                            {
                                errores.Add(new ValidationResult(
                                    $"No existe uno de los subperiodos que esta seleccionando.",
                                    new[] { "SubperiodoId" }));
                            }
                        }
                    }
