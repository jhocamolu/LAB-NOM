...
{
    public class CrearIdiomaRequest : IRequest<CommandResult>, IValidatableObject
    {
        #region Validaciones
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            try
            {
                ...
                var elemento = dbContexto.Idiomas.SingleOrDefault(x => x.Codigo == Codigo);
                if (elemento != null)
                {
                    errores.Add(new ValidationResult(
                       $" El idioma que intentas crear ya existe",
                       new[] { "Codigo" }));
                }
            }
            catch (Exception)
