...
{
    public class CrearSolicitudVacacionRequest : IRequest<CommandResult>, IValidatableObject
    {
        #region Validacion
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            try
            {
                ...
                if (LibroVacacionesId != null)
                {
                    ...
                    {
                        // Consulta el año de trabajo actual
                        var anioTrabajo = dbContexto.AnnoVigencias.FirstOrDefault(x => x.Estado == EstadoAnnoVigencia.Vigente);
                        //Consulta si el periodo de vacaciones tiene más de 360  y 15 días de distfrute
                        var diasLey = dbContexto.ParametroGenerales.FirstOrDefault(x => x.Alias == "diasLey" && x.AnnoVigenciaId == anioTrabajo.Id);
                        if (((int)DiasDisfrute + (int)DiasDinero) > Int32.Parse(diasLey.Valor))
                        {
                            errores.Add(new ValidationResult("Los días registrados en la solicitud  no pueden ser mayor a 15 días.",
                                                        new[] { "snackbar" }));
                        }
                ...
                if (FechaInicioDisfrute != DateTime.MinValue)
                {
                    ...
                    if (FechaInicioDisfrute.Value.Date < primerDiaDelMes.Date)
                    {
                    ...
                    if (validaFechas != null)
                    {
                        foreach (var validaFecha in validaFechas)
                        {
                            ...
                            if ((FechaInicioDisfrute == validaFecha.FechaInicioDisfrute && FechaFinDisfrute != validaFecha.FechaFinDisfrute) ||
                                ...
                                (FechaFinDisfrute == validaFecha.FechaInicioDisfrute || FechaInicioDisfrute == validaFecha.FechaFinDisfrute))
                            {
                                banderaError3 = true;
                            }
                        }
