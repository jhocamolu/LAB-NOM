...
namespace ApiV3.Dominio.Beneficios.Comandos.Crear
{
    ...
    public class CrearBeneficioRequest : IRequest<CommandResult>, IValidatableObject
    {
        #region Validaciones
        #region Elementos
        [Required(ErrorMessage = ConstantesErrores.Requerido)]
            ...
            try
            {
                ...
                if (FuncionarioId != null)
                {
                    ...
                    else if (funcionarioId.Estado == EstadoFuncionario.Activo)
                    {
                        ...
                        if (datosActuales.ContratoId != null)
                        {
                ...
                #region TipoBeneficioId,FuncionarioId
                ...
                #region  validarVecesAño
                if (tipoBeneficio.VecesAnio > 0)
                {


                    var vecesAnio = contexto.Beneficios.Where(x => x.FuncionarioId == FuncionarioId &&
                                                                   x.TipoBeneficioId == TipoBeneficioId &&
                                                                   x.EstadoRegistro == EstadoRegistro.Activo &&
                                                                   x.FechaSolicitud.Year == hoy.Year &&
                                                                   x.Estado != EstadoBeneficiosCorportativos.Cancelada &&
                                                                   x.Estado != EstadoBeneficiosCorportativos.Rechazada
                                                             ).Count();
                    if (vecesAnio >= tipoBeneficio.VecesAnio)
                    {
                        errores.Add(new ValidationResult("El beneficio solicitado excede la cantidad de veces permitidas en el año en curso.", new[] { "snackbarError" }));
                        return errores;
                    }
                }
                #endregion
                ...
                if (FechaSolicitud != null)
                {
                    ...
                    {
                        errores.Add(new ValidationResult(
                        "La fecha que intentas ingresar no debe ser menor que la fecha actual.",
                        new[] { "FechaSolicitud" }));
                    }
                }
                #endregion
                ...
                if (ValorAutorizado == null)
                {
                    ...
                    {
                        errores.Add(new ValidationResult("Rango permitido de 1 -99999999", new[] { "ValorAutorizado" }));
                    }
                }
