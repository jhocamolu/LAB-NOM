...
{
    public class ProcedimientoDinamico : IDynamicProcedure
    {
        ...
        public ProcedimientoDinamico(NominaDbContext contexto, IReadOnlyRepository repositorio)
        {
        ...
        /// <summary>
        /// Ejecucion del procedimiento de nomina
        /// </summary>
        /// <param name="concepto"></param>
        /// <returns></returns>
        public async Task<ConceptoNomina> StoredProcedure(ConceptoNomina concepto, string formula)
        {
            try
            {
                this.listaParametros.Clear();
                this.listaVariables.Clear();
                this.listaValoresEjecucion.Clear();
                this.listaElementosAEjecutar.Clear();
                this.elementoFormula.Clear();
                this.usoInconsistencia = string.Empty;
                ...
                if (formula != null)
                {
                    ...
                    logicaProcedimiento = await CrearProcedimiento(this.ConvertirXml(formula));

                    if (!listaValoresEjecucion.Contains("@Resultado = @ConceptoValor OUTPUT"))
                    {
                        listaValoresEjecucion.Add("@Resultado = @ConceptoValor OUTPUT");
                    }
                    if (!listaValoresEjecucion.Contains("@Inconsistencia = @ConceptoInconsistencia OUTPUT"))
                    {
